...
if not option == 'q':
    ...
    first_model = '0'
    model_count = len(glob.glob1(model_path, '*.0'))
    ...
    if not model_choice == 'q':
        ...
        # with open(sample_path + sample_name + '.txt', 'wb') as Output:
        max_size = 1
        stdout = os.system('python3 -B pytorch-rnn/sample.py --checkpoint '
                           + model_path + model_name + '_' + model_choice +
                           '.json > ' + sample_path + sample_name + '.txt')
        if stdout.tell() > max_size:
            sys.exit()
        # for
        ...
        # os.system('python3 -B pytorch-rnn/sample.py --checkpoint '
